  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 mb-8">
        <div className="grid grid-cols-4 gap-2 h-[45vh]">
          <div className="col-span-2 row-span-2">
            <img src={property.images[0]} alt="Main" className="w-full h-full object-cover rounded-l-xl"/>
          </div>
          {[1, 2].map((index) => (
            <img 
              key={`top-${index}`} 
              ...
              alt={`Image ${index + 1}`} 
              className={`w-full h-full object-cover ${index === 1 ? 'rounded-tr-xl' : ''}`}
            />
          ))}
          {[1, 2].map((index) => (
            <div key={`bottom-${index}`} className="relative">
              <img 
                ...
                alt={`Image ${index + 3}`} 
                className={`w-full h-full object-cover ${index === 2 ? 'rounded-br-xl' : ''}`}
              />
              {index === 2 && (
                <Button 
                  variant="outline" 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between pb-6 border-b">
              <div>
                <h2 className="text-2xl font-semibold">Entire villa hosted by Owner</h2>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-2">
                  {["Check-in", "Check-out"].map((label) => (
                    <div key={label} className="space-y-1 border rounded-t-lg p-3">
